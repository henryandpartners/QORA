...
export const THEMES = [
  ...
  },
  {
    id: 'coralreef',
    name: 'CORAL REEF',
    swatch: ['#ff6b9d', '#ff9e6b', '#ffd4a8'],
    gradient: (t) => [
      0.9 - Math.sin(t * Math.PI) * 0.15,
      0.25 + Math.sin(t * Math.PI) * 0.4 + t * 0.2,
      0.45 + t * 0.35,
    ],
    decoRGB: [0.7, 0.1, 0.25],
    decoHue: 0.95,
    trailHue: { min: 0.9, range: 0.12 },
    scaffold: { outer: 0x2a0a1e, inner: 0x3a0a26, ring: 0xff6b9d, core: 0xff9ec0 },
    bg: '#140510',
    accent: '#ff6b9d',
    accent2: '#ffd4a8',
  },
  {
    id: 'magnetar',
    name: 'MAGNETAR',
    swatch: ['#4665ff', '#9ecfff', '#fff3b0'],
    gradient: (t) => [
      0.3 + Math.sin(t * Math.PI) * 0.25,
      0.45 + t * 0.45,
      0.85 + Math.cos(t * Math.PI) * 0.1,
    ],
    decoRGB: [0.1, 0.15, 0.5],
    decoHue: 0.62,
    trailHue: { min: 0.55, range: 0.18 },
    scaffold: { outer: 0x0a0f2e, inner: 0x101a3e, ring: 0x9ecfff, core: 0xfff3b0 },
    bg: '#050818',
    accent: '#9ecfff',
    accent2: '#fff3b0',
  },
  {
    id: 'goldenhour',
    name: 'GOLDEN HOUR',
    swatch: ['#ffb347', '#ff6bd6', '#8b5cf6'],
    gradient: (t) => [
      0.85 + Math.sin(t * Math.PI) * 0.1,
      0.3 + Math.sin(t * Math.PI) * 0.35 + t * 0.15,
      0.35 + Math.cos(t * Math.PI) * 0.35,
    ],
    decoRGB: [0.6, 0.15, 0.35],
    decoHue: 0.93,
    trailHue: { min: 0.08, range: 0.15 },
    scaffold: { outer: 0x241028, inner: 0x301438, ring: 0xffb347, core: 0xffd4a8 },
    bg: '#120a14',
    accent: '#ffb347',
    accent2: '#ff6bd6',
  },
